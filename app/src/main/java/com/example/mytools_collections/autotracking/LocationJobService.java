...
import static com.example.mytools_collections.MainActivity.JOB_STATE_CHANGED;
import static com.example.mytools_collections.MainActivity.LOCATION_ACQUIRED;
...
//                        Toast.makeText(getBaseContext(),"new point",Toast.LENGTH_SHORT).show();
                    ...
                    if(cd.isConnectingToInternet()) { // check whether internet is available or not
                        ...
                        i1.putParcelableArrayListExtra("points", updatesList);
                            startService(i1); //untuk menginsert lokasi ke databse (atur url di UploadLocationService.class)
                        updatesList.clear();
                        Toast.makeText(getApplicationContext(),"Insert Lokasi Berhasil", Toast.LENGTH_SHORT).show();
                    }else{ // if there is no internet connection
                        updatesList.add(location); // add location points to the list
                        Toast.makeText(getApplicationContext(),"Insert Lokasi Gagal, Cek Koneksi Anda", Toast.LENGTH_SHORT).show();
                    }
                }
            }
    ...
    private void createNotification() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            notification = mBuilder.setSmallIcon(R.drawable.logo2).setTicker("Tracking").setWhen(0)
                    .setAutoCancel(false)
            ...
            notification = mBuilder.setSmallIcon((Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)? R.drawable.logo2:R.mipmap.ic_launcher).setTicker("Tracking").setWhen(0)
                    .setAutoCancel(false)
                    .setCategory(Notification.EXTRA_BIG_TEXT)
                    .setContentTitle("Tracking")
